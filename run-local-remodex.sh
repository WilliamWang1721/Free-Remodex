...
start_bridge() {
  log "Starting bridge"
  cd "${BRIDGE_DIR}"
  # For local OSS testing, keep the bridge in the foreground instead of the
  # macOS launchd service path so relay connectivity and QR output stay visible.
  REMODEX_RELAY="ws://${RELAY_HOSTNAME}:${RELAY_PORT}/relay" \
  REMODEX_RELAY_CONNECT_URL="ws://${RELAY_BRIDGE_HOST}:${RELAY_PORT}/relay" \
